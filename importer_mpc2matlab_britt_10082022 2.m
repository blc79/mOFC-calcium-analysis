...
meta.name = meta_import;

clearvars -except meta;

% Finding number of sessions per file
...
a=a(1:lessOne);
    ...
    %what was the FR for this session? 
   %will need to change this for extinction sessions and 50% reinforced
   %sessions ince the FR is not in the file name 
   
 str=data(j).raw.meta{5,2};
newstr=extractBefore(str,'_');
FR=extractAfter(newstr,'FR');
FR=str2num(FR) ;
data(j).FR = FR; 
typeStr=extractAfter(str,'_');
type = extractBefore(typeStr,'_');

data(j).type = type; 
dayStr = extractAfter(str,'_');
day=extractAfter(dayStr,'_');
day = str2num(day); 
data(j).day = day; 
...
save(strcat('data_',A.animal,'_',A.date),'A');
